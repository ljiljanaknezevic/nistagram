...
func (repo *UserRepository) GetAllUsersExceptLogging(email string) []model.User {
	...
	repo.Database.Where("email != ?", email).Preload("Muted").Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").Find(&users)
	return users
}
func (repo *UserRepository) GetAllUsersExceptLoggingForTag(email string) []model.User {
	...
	var isTrue bool
	isTrue = true
	repo.Database.Where("email != ? and can_tag = ?", email, isTrue).Preload("Muted").Preload("Following").Preload("WaitingFollowers").Preload("Blocked").Preload("Followers").Find(&users)
	return users
}
...
func (repo *UserRepository) UpdateUser(user *model.User) error {
	result := repo.Database.Preload("Muted").Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").Save(user)
	fmt.Println(result.RowsAffected)
	return nil
}
func (repo *UserRepository) DeleteFromWaitingList(ID uint) error {
	repo.Database.Where("ID = ?", ID).Preload("Muted").Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").Delete(&model.WaitingFollower{})
	return nil
}
...
func (repo *UserRepository) UserForLogin(email string) model.User {
	...
	repo.Database.Where("email = ?", email).Preload("Muted").Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").First(&authUser)
	return authUser
}
