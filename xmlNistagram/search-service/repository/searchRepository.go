...
func (repo *SearchRepository) GetAllUsers() []model.User {
	...
	repo.Database.Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").Preload("UsersWhoBlocked").Find(&users)
	return users
}

func (repo *SearchRepository) GetAllUsersExceptLogging(username string) []model.User {
	var users []model.User
	repo.Database.Where("email != ?", username).Preload("Following").Preload("WaitingFollowers").Preload("Followers").Preload("Blocked").Preload("UsersWhoBlocked").Find(&users)
	return users
}
